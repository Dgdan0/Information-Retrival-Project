...
class InvertedIndex:  
    ...
    @staticmethod
    def load_posting_locs(base_dir, bucket_name=None):
        posting_locs = defaultdict(list)
        bucket_id = 0
        while True:
            posting_locs_path = str(Path(base_dir) / f'{bucket_name}{bucket_id:03}_posting_locs.pickle')
            try:
                with open(posting_locs_path, 'rb') as f:  # Using open directly for simplicity
                    current_posting_locs = pickle.load(f)
                    for key, value in current_posting_locs.items():
                        posting_locs[key].extend(value)
                bucket_id += 1
            except FileNotFoundError:
                break  # Exit the loop if no more posting_locs files are found
        print(posting_locs)
        return posting_locs
